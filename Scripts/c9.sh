...
echo ""
echo "----------------------------"
echo "---- Prepare for Task 9 ----"
echo "----------------------------"
echo "#--| "
echo "#--> To create Review we need Place id"
...
echo "#--| "
echo ""
user_output=$(run_console_command 'create User email="bob@hbtn.io" password="bobpwd" first_name="Bob" last_name="Dylan"' | tee /dev/tty)
user_id=$(echo "$user_output" | grep -E "^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$" | tail -1)
echo ""
if [ -z "$state_id" ]; then
    echo "#--| "
...
echo "#--> User ID: $user_id"
echo "#--| "
echo "#--> Creating a new Review"
echo "#--> with Place ID: $place_id"
echo "#--> and User ID: $user_id"
echo "#--| "
echo ""
review_output=$(run_console_command 'create Review place_id="'$place_id'" user_id="'$user_id'" text="Amazing_place,_huge_kitchen"' | tee /dev/tty)
review_id=$(echo "$review_output" | grep -E "^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$" | tail -1)
echo ""
if [ -z "$state_id" ]; then
    echo "#--| "
    echo "#--> Couldn't find a valid Review id"
    echo "#--> Your console is not printing Review id"
    echo "#--> Check do_create"
    echo "#--| "
  exit 1
fi
...
echo "#--| "
echo ""
run_console_command 'all Review'
echo ""
echo "#--| "
echo "#--> Executing SQL query to list all Reviews:"
echo "#--| "
echo ""
run_mysql_command 'SELECT * FROM reviews\G' -h"$HBNB_MYSQL_HOST" -u"$HBNB_MYSQL_USER" -p"$HBNB_MYSQL_PWD" "$HBNB_MYSQL_DB"
echo ""
echo "#--| "
echo ""
