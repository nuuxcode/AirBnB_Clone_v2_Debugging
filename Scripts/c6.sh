...
echo "#--| "
echo ""
run_console_command 'all State'
echo ""
echo "#--| "
echo "#--> Executing SQL query to list all States:"
echo "#--| "
echo ""
run_mysql_command 'SELECT * FROM states\G' -h"$HBNB_MYSQL_HOST" -u"$HBNB_MYSQL_USER" -p"$HBNB_MYSQL_PWD" "$HBNB_MYSQL_DB"
echo ""
echo "#--| "
...
echo "#--| "
echo ""
run_console_command 'all City'
echo ""
echo "#--| "
echo "#--> Creating another new City:"
echo "#--| "
echo ""
city_output=$(run_console_command 'create City state_id="'$state_id'" name="San_Jose"' | tee /dev/tty)
city_id=$(echo "$city_output" | grep -E "^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$" | tail -1)
echo ""
if [ -z "$city_id" ]; then
    echo "#--> Couldn't find a valid id city id"
...
echo "#--| "
echo ""
run_mysql_command 'SELECT * FROM cities\G' -h"$HBNB_MYSQL_HOST" -u"$HBNB_MYSQL_USER" -p"$HBNB_MYSQL_PWD" "$HBNB_MYSQL_DB"
echo ""
echo "#--| "
echo ""
